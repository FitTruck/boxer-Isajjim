...
#### C-3. 죽은 의존성·실험 코드 제거 (이미지 경량화 + 표면적 축소)
- **무엇**: `ultralytics`(+전이 의존성 scipy/matplotlib/pandas)·`requests` 제거(import 0건), SAM3 백엔드·P1 sdp 스위치·결론 난 실험 스크립트 2종 삭제, 검출기 출력/스크립트 부트스트랩 공용 모듈화(`detection_common.py`/`eval_common.py`).
- **원본 대비**: 서빙 이미지에서 미사용 ML 스택(ultralytics 계열)을 들어내 빌드/콜드 배포 경량화. 행동 보존은 실모델 A/A 2회(77레코드×15필드 0 diff)로 증명.
- **실측 효과**: 순감소 **399줄**. 이미지 크기 절감은 측정 전(CI Docker 빌드는 제거 후에도 성공 — run 27316937962). depth/추론 핫패스 영향 없음(병목은 GPU).
- **근거**: `requirements.txt`, commit `0e3e875`/`0a50a5e`, 머지 `de7b316`
- **날짜**: 2026-06-11 · **분류**: C
